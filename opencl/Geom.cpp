...
bool clip_to_box(const FloatSegment& s, const float& d, FloatSegment* new_s) {
  ...
  if (valid)
    *new_s = FloatSegment(make_floatn(x, y), s.b());
  return valid;
}
...
floatn line_intersection(const FloatSegment& a, const FloatSegment& b,
                         bool* lines_intersect, bool* segs_intersect) {
...
void v_segs(const FloatSegment& a, const FloatSegment& b,
  ...
  float dist = length(a.a()-c);
  FloatSegment aa = a;
  FloatSegment bb0(c, b.a());
  FloatSegment bb1(c, b.b());
  ...
  if (dist_ < dist) {
    c = c_;
    dist = dist_;
    aa = FloatSegment(a.b(), a.a());
    bb0 = FloatSegment(c, b.a());
    bb1 = FloatSegment(c, b.b());
  }
  ...
  if (dist_ < dist) {
    c = c_;
    dist = dist_;
    aa = FloatSegment(b.a(), b.b());
    bb0 = FloatSegment(c, a.a());
    bb1 = FloatSegment(c, a.b());
  }
...
vector<floatn> v_sample(const FloatSegment& a, const FloatSegment& b) {
...
//
//
// Fit boxes in cases where the line segments don't intersect.
// Pass A and B by value! They'll change in the function.
void FitBoxesNoIntersection(FloatSegment A, FloatSegment B, const float& min_d,
                            std::vector<floatn>* samples,
                            vector<floatn>* origins, vector<float>* lengths) {
  static const int OFFSET_FACTOR = 4;

  const FloatSegment A_orig(A);
  const FloatSegment B_orig(B);
  bool done = false;
  while (!done) {
    ...
    if (dist(ca, cb) < min_d) {
      ...
      const FloatSegment C(ca, ca + make_floatn(cx_dir, cy_dir));
      floatn c_cc;
      bool c_cc_end;
    ...
    if (c_cb_end) {
      ...
      const FloatSegment B0(c_cb, B.a());
      const FloatSegment B1(c_cb, B.b());
    ...
    if (dot(A.unit(), B.unit()) <= 0) {
      ...
      A = FloatSegment(make_floatn(A_x, A_y), A.b());
      B = FloatSegment(make_floatn(B_x, B_y), B.b());
    }
  }
}
...
void FitBoxes(FloatSegment A, FloatSegment B, const float& min_d,
  ...
  if (ca == cb) {
    ...
    if (B.a().x > B.b().x)
      B.reverse();
    FloatSegment A0_orig(ca, A.a());
    FloatSegment A1_orig(ca, A.b());
    FloatSegment B0_orig(cb, B.a());
    FloatSegment B1_orig(cb, B.b());
    FloatSegment A0, A1, B0, B1;

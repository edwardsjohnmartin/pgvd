...
inline bool weakCompareLineInfo(LinePair *a, LinePair *b) {
	...
	if (a->k1_even != b->k1_even) return false;
	if (a->k1_odd != b->k1_odd) return false;
	if (a->k2_even != b->k2_even) return false;
	if (a->k2_odd != b->k2_odd) return false;
	if (a->num_samples != b->num_samples) return false;
	if (a->p_origin.x != b->p_origin.x) return false;
	if (a->p_origin.y != b->p_origin.y) return false;
#ifdef OCT3D
	if (a->p_origin.z != b->p_origin.z) return false;
#endif
	if (a->s0 != b->s0) return false;
	if (a->s1 != b->s1) return false;
	if (a->u.x != b->u.x) return false;
	if (a->u.y != b->u.y) return false;
#ifdef OCT3D
	if (a->u.z != b->u.z) return false;
#endif
	return true;
}

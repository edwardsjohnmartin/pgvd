...
// The following defines were previously set during runtime compilation, 
// see reduction.cpp in the opencl reduction demo
// #define T float
...
/*
    ...
    isn't needed
    EDIT: Nvidia Only from here on out!
*/
...
__kernel void reduce6(__global T *g_idata, __global T *g_odata, unsigned int n, __local volatile T* sdata)
{
    ...
    unsigned int tid = get_local_id(0);
    unsigned int i = get_group_id(0)*(get_local_size(0)*2) + get_local_id(0);
    unsigned int gridSize = blockSize6*2*get_num_groups(0);
    sdata[tid] = 0;

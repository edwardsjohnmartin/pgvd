...
class FloatSegment {
 public:
  FloatSegment() {}
  FloatSegment(const floatn& a_, const floatn& b_)
      : _a(a_), _b(b_) {}
  ...
  friend std::ostream& operator<<(std::ostream& out, const FloatSegment& s);
  friend std::istream& operator>>(std::istream& in, FloatSegment& s);
...
inline std::ostream& operator<<(std::ostream& out, const FloatSegment& s) {
  out << s.a() << " " << s.b();
  return out;
}

inline std::istream& operator>>(std::istream& in, FloatSegment& s) {
  in >> s._a >> s._b;
  return in;
}
